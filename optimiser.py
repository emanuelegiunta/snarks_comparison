...
def mkt_gen_old(h):
	...
	#h = heigth of the tree, the longest path includes h+1 nodes
	root = mkt_node()
	if h > 0:
		root.next.append( mkt_gen(h - 1) )
		root.next.append( mkt_gen(h - 1) )

	if h == 0:
		root.next.append( mkt_gen(-1) )
	return root
 
def mkt_reset_old(root, key):
	#set the key value to key
	root.key = key
	for child in root.next:
		mkt_reset_old(child, key)

def mkt_single_cost_old(root, path_num, no_collision = False):
	#assumes the root is clean
	#explore the tree for path_num times, marking visited nodes and
	#	returning the total number of visited nodes. If no_collision
	#	is true, the final leaves are all different, which is used in
	#	Ligero when quering a subset Q of t elements. However in FRI
	#	the repetions can potentially collide
	out, i = 0, 0
	#
	while i < path_num:
		flag = False
		node = root
		while node.next: #the list is not empty
			# I don't need anymore the father's key (from -1 to 1)
			if node.key == -1:
				out -= 1
			node.key = 1

			for child in node.next:
				if child.key == 0:	#the node was never visited
					child.key = -1	#request the hash for that node (from 0 to -1)
					out += 1		#count one more hash 

			node = rnd.choice(node.next)
			if node.key == 0: 	#the node was not visited before
				flag = True 	#We found a new path

		if flag or (not no_collision):
			i += 1

	#
	mkt_reset_old(root, False)
	return out

def mkt_avg_cost_old(leaf_num, path_num):
	#h is the heigth of the tree, i.e. the leaf are 2^h
	#	technically the height is h+1 since the leaves have only one ancestor

	out = 0.0
	root = mkt_gen_old( int(math.ceil(np.log2(leaf_num))) )

	print_message("Generating MKT of height %d" % int(math.ceil(np.log2(leaf_num))) )

	for i in range(MKT_ITER):
		out += mkt_single_cost_old(root, path_num)
